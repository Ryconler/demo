	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == 200) {
				...
				const lingquInfo = {
					name: userWechat.nickname,
					mobile: userWechat.mobile,
					city: userWechat.location,
					address: userWechat.address,
				};
				localStorage.setItem("lingquInfo", JSON.stringify(lingquInfo));
			}
		}
	})
}
